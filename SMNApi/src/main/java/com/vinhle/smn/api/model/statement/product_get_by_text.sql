CREATE DEFINER =`root`@`localhost` PROCEDURE `product_get_by_text`(IN _text TEXT)
  BEGIN
    ...
    WHERE (product.product_name LIKE search OR product.product_code LIKE search)
    ORDER BY product.is_active, product.product_type_id;
  END