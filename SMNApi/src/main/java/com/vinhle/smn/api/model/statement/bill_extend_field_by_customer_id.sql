CREATE DEFINER =`root`@`localhost` PROCEDURE `bill_extend_field_by_customer_id`(IN _customer_id INT)
  BEGIN
    SELECT
      bill.*,
    ...
    WHERE bill.customer_id = _customer_id
    ORDER BY bill.modified_date DESC;
  END